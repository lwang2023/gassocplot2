...
plot_recombination_rate <- function(chr, x.min, x.max, build=37) {
  ...
  recomb.plot <- ggplot(data=recomb.df, mapping=aes(x=coordinates, y=y)) + geom_line(colour="steelblue1") + scale_y_continuous(breaks=c(0,25,50,75,100), limits=c(0,100)) + xlab(NULL) + scale_x_continuous(limits=c(x.min,x.max), breaks=NULL) + ylab("Recomb. Rate") + theme_bw() + theme(axis.title.y=element_text(vjust=1.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(axis.title=element_text(size=16), axis.text=element_text(size=14)) + theme(panel.background = element_rect(fill = NA))
  return(recomb.plot)
}
...
plot_assoc <- function(data, corr=NULL, corr.top=NULL, x.min, x.max, top.marker=NULL, ylab, type="log10p", labels=NULL, sig.thres=NULL, point.padding=0.15, nudge_x=0, nudge_y=0, ylim_prob1=TRUE, ylim=14){
  ...
  if(type=="prob" & ylim_prob1){ylim <- max(ylim,1)}
...
plot_assoc_stack <- function(data, corr=NULL, corr.top=NULL, x.min, x.max, top.marker=NULL, ylab, type="log10p", labels=NULL, sig.thres=NULL, point.padding=0.15, nudge_x=0, nudge_y=0, ylim_prob1=TRUE, ylim=14){
  ...
  if(!is.null(sig.thres) & type=="log10p"){ylim <- max(ylim, (-log10(sig.thres) + -log10(sig.thres)*0.1))}
  if(type=="prob" & ylim_prob1){ylim <- max((max(data$stats)+0.2*max(data$stats)),1)}
...
stack_assoc_plot <- function(markers, z, corr=NULL, corr.top=NULL, traits, ylab=NULL, type="log10p", x.min=NULL, x.max=NULL, top.marker=NULL, legend=TRUE, build=37, labels=NULL, sig.thres=NULL, point.padding=0.15, nudge_x=0, nudge_y=0, ylim_prob1=TRUE, ylim=14){
  ...
  if(length(names(markers))!=3) stop("dataset needs to include marker, chr and pos columns in that order")
